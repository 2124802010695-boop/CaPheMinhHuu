...
import { staffLoginAPI } from '../../common/services/authService';
      ...
      if (res && res.token) {
        ...
        if (res.isFirstLogin) {
          ...
          if (res.user.role === "Cashier") {
            // Kiểm tra ca hiện tại trước khi vào POS
            try {
              const shiftRes = await getCurrentShiftAPI();
              // API trả { message: "Chưa có ca", shift: null } khi chưa có ca
              // hoặc trả shift object khi có ca Open
              ...
              // Lỗi khi check shift → mở shift-open cho an toàn
              navigate("/cashier/shift-open");
            }
