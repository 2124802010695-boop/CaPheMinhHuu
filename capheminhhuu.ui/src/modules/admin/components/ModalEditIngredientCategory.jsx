...
import { updateIngredientCategoryAPI } from '../services/ingredientCategoryService';

const ModalEditIngredientCategory = ({ open, handleClose, category, fetchCategories }) => {
    ...
    const handleSubmit = async () => {
        ...
        setLoading(true);
        try {
            await updateIngredientCategoryAPI(category.id, { name: form.name.trim(), description: form.description });
            ...
            <DialogTitle sx={{ bgcolor: '#10b981', color: 'white', py: 2 }}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <EditIcon />
                        <Typography variant="h6" fontWeight="bold">Chỉnh Sửa Danh Mục Nguyên Liệu</Typography>
                    </Box>
...
export default ModalEditIngredientCategory;
